...
function sumNumbers(array) {
  return _.compact(array).reduce( (sum, num) => sum + num, 0)
}
...
function countClassesByInstructor(collection, instructor) {
  let res = _.reduce( collection, function( acc, ele) { return acc + (ele.instructor === instructor ? 1 : 0) }, 0 )
  return res ? res : "There is no instructor by that name.";
}
...
function getUniqueClasses(collection) {
  let condensedObjectsArray =  _.uniq( collection.map( obj => _.pick( obj, ["title", "priceInCents"])) );
  return _.uniqWith(condensedObjectsArray, _.isEqual);
}
...
function orderClassesByTitleAndLevel(collection){
  return _.orderBy(collection, ['style', 'level'], ['asc', 'desc']).map( object => _.pick(object, ["title", "instructor", "level"]));
}
